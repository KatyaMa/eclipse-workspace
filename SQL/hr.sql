...
-- Let utilize the IN or NOT IN operator. You can rewrite and combine the queries above as follows:
SELECT   employee_id, first_name, last_name, department_id
FROM employees
...
-- The following example uses a subquery with the NOT IN operator
...
-- Let's utilize the comparison operator (=, >, =, <= ,!=). The following example finds the employees who have the highest salary:
...
-- The following query finds employees whose salary is higher than the average salary throughout the company;
...
-- all departments have at least one employee with a salary greater than 10,000. We will use the Where clause and the EXISTS operator.
...
-- all departments that do not have an employee with a salary greater than 10,000, We will use the Where clause, and the NOT EXISTS operator.
SELECT department_name
FROM departments d
